...
open (OUTPUTFILE,"> $outPath") || die "Couldn't open $outPath. Check folder permissions.\n";
...
open (DUMPFILE,"> $dumpPath") || die "Couldn't open $dumpPath.";
...
for my $caseline (@casesFile)
{
    ...
    # this fisher test fails if <pc> is set to a low value.  This is because some
    # samples will be counted more than once, so np1 will be higher than npp
    #
    ...
    # so use the count of unique identical matches
    my $fish_ident = scalar @$identicalMatchIDs;
    my $fish_identCont = scalar @$identicalMatchIDsCont;

    my $npp = $totalCases + $totalCont;        #$PresCase+$AbsCase+$PresCont+$AbsCont;
 	my $n1p = $fish_ident + $fish_identCont;     #$PresCase+$PresCont;
 	my $np1 = $fish_ident + $nonMatch;          #$PresCase+$AbsCase;
 	my $n11 = $fish_ident;                      #$PresCase;
...
sub findMatches
{
	...
	for my $key_sample (keys %$hash_ref)
	#for my $key_sample (keys %hash)
	{
		# check each position within each individual for a match
		for my $key_position (keys %{$hash_ref->{$key_sample}{$state}{$chr}})
		#for my $key_position (keys %{$hash{$key_sample}{$state}{$chr}})
		{
			...
			my @testline = split(/\s+/, $hash_ref->{$key_sample}{$state}{$chr}{$key_position});
