...
=head1 This script analyses copy number variation (CNV) files to identify rare CNV's.
 ...
 -d --dump [file]       Dump file path (defaults to <Outfile>.matches_dump.zip)
 ...
 -g --greedy            Include 'Non-Identical Larger' match types when counting
                        positive cases and controls, and include a list of matched individuals.

 Basic usage:
 ============
 CNV_counter.pl --case CaseFile.txt --control ControlFile.txt

 Usage with optional parameters:
 ===============================
 CNV_counter.pl -c CaseFile.txt -n ControlFile.txt -o OutputPath -d DumpPath -m 85 -f 0.1 -g

 See the comments in this file for the format of input and output files and
 descriptions of match types.

=cut

# This script analyses copy number variation (CNV) files to identify rare CNV's.
# By David A Fasel daf2139<at>columbia.edu 08/2014
...
#     'identical' match
# * NILarger_ID_Matches_Cases / _Ctrls:
#     Comma separated list of individual's that have at least one
#     'Non-identical Larger' match.  Only shown if --greedy flag is on.
#
# Format of Dump File:
# ====================
# Case CNV Position
...
my @headers = qw(
    ...
    ID_Matches_Cases  ID_Matches_Ctrls
);
if ($GREEDY) {
    splice @headers, -1, 0, 'NILarger_ID_Matches_Cases';
    push @headers, 'NILarger_ID_Matches_Ctrls';
}
...
{
    my ($caseMatches, $ctrlMatches, $identicalMatchIDs, $identicalMatchIDsCont, $NILargerMatchIDs, $NILargerMatchIDsCont);
    ...
    $identicalMatchIDs = $caseMatches->{'identicalMatchIDs'};
    $identicalMatchIDsCont = $ctrlMatches->{'identicalMatchIDs'};

    if ($GREEDY) {
        $NILargerMatchIDs = $caseMatches->{'NILargerMatchIDs'};
        $NILargerMatchIDsCont = $ctrlMatches->{'NILargerMatchIDs'};
    }

    my $cases = scalar @$identicalMatchIDs;
    my $controls = scalar @$identicalMatchIDsCont;
    
    # if greedy, the number of postive matches needs equals each individual
    # who had at least one matching CNV that was either "identical or "non-identical larger".
    # Since an individual could (rarely) have a match in each category, we need to ensure 
    # that we only count each matching individual once 
    if ($GREEDY) {
        my %matching;
        $matching{$_} = 1 for @$identicalMatchIDs;
        $matching{$_} = 1 for @$NILargerMatchIDs;
        $cases = scalar(keys %matching);
        undef %matching;
        $matching{$_} = 1 for @$identicalMatchIDsCont;
        $matching{$_} = 1 for @$NILargerMatchIDsCont;
        $controls = scalar(keys %matching);
    }
    ...
    if (scalar @$identicalMatchIDs == 0)
        {@$identicalMatchIDs = ("none") }
    elsif ($caseFreq >= $FREQ_FILTER_CASE)
        {@$identicalMatchIDs = ("exceeds_freq_filter")}

    if (scalar @$identicalMatchIDsCont == 0)
        {@$identicalMatchIDsCont = ("none") }
    elsif ($controlFreq >= $FREQ_FILTER_CTRL)
        {@$identicalMatchIDsCont = ("exceeds_freq_filter")}

    if ($GREEDY) {
        if (scalar @$NILargerMatchIDs == 0)
            {@$NILargerMatchIDs = ("none") }
        elsif ($caseFreq >= $FREQ_FILTER_CASE)
            {@$NILargerMatchIDs = ("exceeds_freq_filter")}

        if (scalar @$NILargerMatchIDsCont == 0)
            {@$NILargerMatchIDsCont = ("none") }
        elsif ($controlFreq >= $FREQ_FILTER_CTRL)
            {@$NILargerMatchIDsCont = ("exceeds_freq_filter")}
    }
    ...
    push (@output, join(",", sort @$NILargerMatchIDs)) if $GREEDY;
    push (@output, join(",", sort @$identicalMatchIDsCont));
    push (@output, join(",", sort @$NILargerMatchIDsCont)) if $GREEDY;
...
sub findMatches
{
    my (%Matches, @matchesForDumpfile, %identicalMatchIDs, %NILargerMatchIDs, $matchtype);
    ...
    for my $hash_state (keys %$hash_ref) {
        ...
        for my $node (@$matching_nodes) {
            ...
            # For definitions of these matches, see comments at the top of this file.
                ...
                if ($GREEDY) {
                    $NILargerMatchIDs{$testSampleID} = 1;
                }
            }
    ...
    if ($GREEDY) {
        my @NILargerMatchIDs = keys %NILargerMatchIDs;
        $Matches{NILargerMatchIDs} = \@NILargerMatchIDs;
    }
