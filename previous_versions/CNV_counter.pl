...
CNV_hunter2.pl 
 ...
 -control <Optional: control cnv file, white space delimited> 
 -pc <percent for match to be considered "identical" (defaults to 90)> 
 -out <Output File Path (defaults to Casefile.matches.txt)>
 -dump <dumpPath (defaults to Output.matches_dump.txt)>  
 -freq <Filter matches in dumpfile (defaults to 100 - no filtering)>
...
$PERCENT = ($PERCENT)/100; 
my $freqFilter = $params{"-freq"} || 100;
$freqFilter = ($freqFilter)/100; 
...
close CASE;
chomp (@casesFile);
my %cases = &HashCNV(\@casesFile, "Case");
...
close CONTROL;
chomp (@controlsFile);
my %controls = &HashCNV(\@controlsFile, "Control");
...
for my $caseline (@casesFile) 
{
	...
	# if the line was bad, &findMatches returns false, so go to next line    
	...
	if ($freqFilter >= $caseFreq)
	{
		print DUMPFILE @$dumplines;
	}
	if ($freqFilter >= $controlFreq) {
...
sub HashCNV ($)
{
	...
	foreach my $CNVline (@lines) 
	{
		...
		if ($sample && $chr && $pos && $state){ 
	    	...
	    	print "Warning:  $_[1] file line $counter is missing data and was skipped. ";
	    	print "None of these fields can be 0 or empty: sample:$sample, state:$state, chr:$chr, pos:$pos\n";
	    }
